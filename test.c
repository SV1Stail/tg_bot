...
double get_stack_value(stack * input_list, double x, int *flag) {
    ...
    while (ptr != NULL) {
        switch (ptr->action) {
            case 'd': push_stack(&stack_list,  'd', ptr->digit); break;
            case 'x': push_stack(&stack_list,  'x', x); break;
            case '+': pop_stack(&stack_list, &a, &d); pop_stack(&stack_list, &a, &d2);
                push_stack(&stack_list,  'd', d2+d); break;
            case '*': pop_stack(&stack_list, &a, &d);  pop_stack(&stack_list, &a, &d2);
                push_stack(&stack_list,  'd', d2*d);  break;
            case '/': pop_stack(&stack_list, &a, &d); pop_stack(&stack_list, &a, &d2);
                push_stack(&stack_list,  'd', d2/d); break;
            case '-': pop_stack(&stack_list, &a, &d); pop_stack(&stack_list, &a, &d2);
                push_stack(&stack_list, 'd', d2-d); break;
            case '^': pop_stack(&stack_list, &a, &d); pop_stack(&stack_list, &a, &d2);
                push_stack(&stack_list, 'd', pow(d2, d)); break;
            case 's': pop_stack(&stack_list, &a, &d); push_stack(&stack_list, 'd', sin(d)); break;
            case 'c': pop_stack(&stack_list, &a, &d); push_stack(&stack_list, 'd', cos(d));  break;
            case 't': pop_stack(&stack_list, &a, &d); push_stack(&stack_list, 'd', tan(d));  break;
            case 'g': pop_stack(&stack_list, &a, &d); push_stack(&stack_list, 'd', 1/tan(d)); break;
            case 'q': pop_stack(&stack_list, &a, &d); push_stack(&stack_list, 'd', sqrt(d)); break;
            case 'l': pop_stack(&stack_list, &a, &d); push_stack(&stack_list, 'd', log(d)); break;
        }
        ptr = ptr->next;
    }
    double ddd = 0;
